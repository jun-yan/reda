...
---
```{r eventPlotStrat}
plotEvents(reSurv(t.stop, id, event, death) ~ sex + chemo,
...
- General class of hazards regression
...
- This motivates us to extend methods for right-truncated survival data [@wang1989semiparametric to construct estimating equation.
\end{frame}
...
$$M_i^*(t) = N_i^*(t) - \int_0^tR_i^*(u)\dif H(u), $$
for $R_i^*(t) = \sum_{j=1}^{m_i}\mathbbm{1}\{t_{ij}^*\le t\le Y_i^*\}$, 
$H(t) = \log \Lambda_0(t)$.

- $M_i^*(t)$ is a zero mean stochastic process, we have 
\[
\E\left\{ \sum_{i=1}^n\int_0^t\dif M_i^*(u) \right\}=0
\]
and 
\[
...
- Panel count data (interval censored recurrent event data)
